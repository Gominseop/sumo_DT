...
if __name__=="__main__":
    ...
    base_time = datetime.datetime(2024, 4, 10, 10, 0, 0)
    t_step = 1
    if road_ids:
    ...
    sumogen.generate_route_file(f'{test_file}_route.rou.xml',
                                f'{test_file}_flow.rou.xml',
                                f'{test_file}_detector.xml',
                                f'{test_file}_traffic.txt',
    ...
    t = time.time()
    sumogen.make_sumocfg(f'{test_file}.sumocfg',
                         0, (t_step+fb+bb)*interval+1,
                         summary_path=f'{test_file}_summary.xml', summary_period=3600,
                         edge_path=f'{test_file}.add.xml', edge_period=3600)
    s = time.time() - t
    out["sumo config"] = f'{test_file}.sumocfg'
    out["generate route"] = s

    sumogen.run_sumo(f'{test_file}.sumocfg')
