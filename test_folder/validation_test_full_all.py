...
def validation_volume(test_file, answer_df, s, d, t):
    ...
    result = pd.DataFrame(columns=["date", "time", "road", "seed", "result_volume", "answer_volume",
    ...
    for edge in edges:
        ...
        if i in targets:
            ...
            if sqv >= 0.9:
                ...
                sg = 3
            result.loc[j] = [d, t, i, s, volume, an, geh, gg, sqv, sg]
            j += 1
    ...
    for tt in range(10, 12, 1):
        for td in range(4, 5, 1):
            ...
            for s in range(1, 101, 1):
                test_file = f"result_full_folder/{s}.test_full_{td}_{tt}_edge.xml"
                com_vol = validation_volume(test_file, target_answer, s, td, tt)
                if vv is None:
                    vv = com_vol
                else:
                    vv = pd.concat([vv, com_vol])

    vv.to_csv(f"volume_validation_test_full_all.csv", encoding='cp949', index=False)
