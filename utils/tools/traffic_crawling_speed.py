...
url = "https://utis.pohang.go.kr/statistics/traffic02"
...
# 0 ~ 15
# 1(형산만), 5, 12(형산만), 13, 14, 15
rns = ['동해고속도로', '동해안로', '삼흥로', '새마을로', '새만금포항고속도로', '새천년대로', '영일만대로',
       '일반국도14호선', '일반국도20호선', '일반국도28호선', '일반국도31호선', '일반국도7호선',
       '중앙로', '중흥로', '포스코대로', '희망대로']
...
for r in rnt:
    ...
    for d in range(dr):
        ...
        for index, value in enumerate(rows):
            ...
            df.loc[i] = tlist
            i += 1
df.to_csv(f'C:/Users/FILAB/Desktop/arrange/5_code/SUMO/sumo_DT/utils/traffic_data/속도_2024_{m + 1}.csv', encoding='cp949', index=False)
